...
export default function TipologiasTab() {
  const { data: items, isLoading, mutate } = useSWR('tipologias-config', () => getTypologies());
  ...
  const openEdit = (item: any) => { setEditItem(item); setName(item.name); setOpen(true); };

  const handleSave = async () => {
    if (!name.trim()) return;
    setSaving(true);
    try {
      if (editItem) await updateTypology(editItem.id, { name: name.trim() });
      else await createTypology({ name: name.trim() });
      setOpen(false);
      mutate();
  ...
  const handleDelete = async (id: string) => {
    try { await deleteTypology(id); mutate(); } catch { alert('Error al eliminar'); }
  };
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Tipologías</CardTitle>
          <Button size="sm" onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Nueva Tipología</Button>
        </div>
      </CardHeader>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editItem ? 'Editar Tipología' : 'Nueva Tipología'}</DialogTitle></DialogHeader>
